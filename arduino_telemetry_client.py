...
class TelemetryClient:
    ...
    def get_gpu_metrics(self):
        ...
        # Try NVML first (NVIDIA GPUs)
        if self.gpu_initialized and NVML_AVAILABLE:
            try:
                ...
                print(f"NVML GPU: {gpu_usage}% usage, {gpu_memory_percent:.1f}% memory, {gpu_temp}°C")
        ...
        if GPU_AVAILABLE:
            try:
                ...
                if gpus:
                    ...
                    gpu_usage = gpu.load * 100 if gpu.load is not None else 0
                    gpu_memory_percent = (gpu.memoryUsed / gpu.memoryTotal) * 100 if gpu.memoryTotal > 0 else 0
                    gpu_temp = gpu.temperature if gpu.temperature is not None else 45
                    gpu_name = gpu.name if gpu.name else "Unknown GPU"
                    
                    print(f"GPUtil GPU: {gpu_usage}% usage, {gpu_memory_percent:.1f}% memory, {gpu_temp}°C")
                    return gpu_usage, gpu_memory_percent, gpu_temp, gpu_name
                    
            except Exception as e:
                print(f"GPUtil error: {e}")
        
        # Fallback: Use WMI or estimated values
        try:
            if WMI_AVAILABLE:
                import wmi
                w = wmi.WMI()
                for gpu in w.Win32_VideoController():
                    if gpu.Name and "Microsoft" not in gpu.Name and "Basic" not in gpu.Name:
                        gpu_name = gpu.Name[:20]
                        # Estimate GPU usage based on CPU usage
                        cpu_percent = psutil.cpu_percent(interval=None)
                        gpu_usage = min(cpu_percent * 0.6, 100)  # Estimate 60% of CPU load
                        gpu_memory_percent = 25  # Estimate 25% memory usage
                        gpu_temp = 45 + (gpu_usage * 0.3)  # Estimate temperature
                        
                        print(f"WMI GPU (estimated): {gpu_usage:.1f}% usage, {gpu_memory_percent}% memory, {gpu_temp:.1f}°C")
                        return gpu_usage, gpu_memory_percent, gpu_temp, gpu_name
        except Exception as e:
...
def main():
    """Main entry point"""
    print("Arduino Telemetry Client")
    print("========================")
